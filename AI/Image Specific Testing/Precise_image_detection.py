...
class DatasetImageMatcher:
    def __init__(self, dataset_path="Data_Set", algorithm="auto"):
        ...
        self.categories = []
        self.image_subfolders = {}  # category -> list of image subfolders
    ...
    def load_dataset(self):
        """Load and analyze the dataset structure, including image subfolders and all images"""
        ...
        for i, category in enumerate(self.categories, 1):
            ...
            # Find image subfolders (image1, image2, ...)
            subfolders = [d for d in os.listdir(category_path)
                         if os.path.isdir(os.path.join(category_path, d)) and d.startswith('image')]
            subfolders.sort(key=lambda x: int(x.replace('image', '')))
            self.image_subfolders[category] = subfolders
            print(f"  {i}. {category}: {len(subfolders)} image subfolders")
            for subfolder in subfolders:
                subfolder_path = os.path.join(category_path, subfolder)
                image_files = [f for f in os.listdir(subfolder_path)
                              if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp'))]
                print(f"      - {subfolder}: {len(image_files)} images (should be 16)")
    ...
    def extract_dataset_features(self):
        """Extract features from all images in every image subfolder"""
        print(f"Extracting features from dataset images using {self.algorithm} algorithm...")
        for category in self.categories:
            category_path = os.path.join(self.dataset_path, category)
            self.dataset_features[category] = []
            subfolders = self.image_subfolders.get(category, [])
            for subfolder in subfolders:
                subfolder_path = os.path.join(category_path, subfolder)
                image_files = [f for f in os.listdir(subfolder_path)
                              if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp'))]
                print(f"Processing {category}/{subfolder} ({len(image_files)} images)...")
    ...
    def capture_and_match(self):
        """Capture image from webcam and match against dataset, always saving log and image in specified folders"""
        ...
        while True:
            ...
            if not ret:
                ...
                continue
            capture_count += 1
            capture_path = os.path.join(image_dir, f'captured_for_matching_{capture_count}.jpg')
            ...
            if success:
                ...
                print(f"Matching results saved to matching_results.txt.")
            else:
    ...
    def save_results(self, image_path, matches, conclusion):
        """Save matching results to a file in Image Specific Testing folder, including image subfolder name"""
        try:
            results_file = os.path.join(os.path.dirname(__file__), "matching_results.txt")
            with open(results_file, 'a', encoding='utf-8') as f:
                ...
                for i, match in enumerate(matches[:5]):
                    percentage = match['similarity'] * 100
                    subfolder = match.get('subfolder', 'N/A')
                    f.write(f"  {i+1}. {match['category']} / {subfolder}: {percentage:.1f}% ({match['filename']})\n")
                f.write("-" * 50 + "\n")
